...
void run_table_tests(void) {
    ...
    TEST_CHECK(tableSet(&table, key1, NUMBER_VAL(1)));       /* new key */
    TEST_CHECK(!tableSet(&table, key1, NUMBER_VAL(2)));      /* overwrite, not new */
    TEST_CHECK(tableGet(&table, key1, &out));
    ...
    TEST_CHECK(!tableGet(&table, key1, &out));
    TEST_CHECK(!tableDelete(&table, key1)); /* already gone */

    /* re-set after delete should reuse the tombstone slot, not double-count */
    TEST_CHECK(tableSet(&table, key1, NUMBER_VAL(99)));
    TEST_CHECK(tableGet(&table, key1, &out));
    TEST_CHECK(AS_NUMBER(out) == 99);

    /* growth: enough distinct keys to force at least one resize */
    Table big;
    initTable(&big);
    char buf[16];
    for (int i = 0; i < 200; i++) {
    ...
    /* string interning: copyString twice with same content -> same pointer */
